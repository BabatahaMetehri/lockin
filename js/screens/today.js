...
function buildLogger(date, refresh) {
  ...
  // ---- LIVE autocomplete: type "ri" → all rice options ----
  ...
  function clearAll() {
    ...
    adder.innerHTML = ""; adder.hidden = true;
  }
  ...
  q.addEventListener("input", () => {
    ...
    dropdown.innerHTML = "";
    if (!items.length) {
      dropdown.appendChild(el("p.muted", { text: "No match. Add it manually below ↓", style: "padding:10px;margin:0;font-size:.85rem" }));
    } else {
      items.forEach((it) => dropdown.appendChild(suggestionRow(it)));
    }
  ...
  q.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Enter" && dropdown.firstChild && !adder.hidden === false) { e.preventDefault(); dropdown.firstChild.click(); }
  });
  ...
  function openAdder(it) {
    ...
    const addBtn = el("button.btn big", { text: "✓ Log this", onclick: () => {
      ...
      const k = Math.round(it.kcal * g / 100);
      const p = Math.round(it.protein * g / 100 * 10) / 10;
      addCustomMeal(date, { name: it.name, grams: g, kcal: k, protein: p });
      q.value = ""; clearAll(); renderList(); tap();
      q.focus();
    }});
    ...
    adder.appendChild(el("div", { style: "display:flex;gap:8px;align-items:center;flex-wrap:wrap" }, [
      grams, el("span.muted", { text: "grams" }),
      el("div", { style: "flex:1;min-width:80px" }), addBtn, cancel,
    ]));
    adder.appendChild(preview);
    setTimeout(() => grams.select(), 50);
  }

  // ---- manual entry (fallback for foods not in the DB) ----
